...
def name_match(tidal_track, spotify_track):
    ...
    return simple_spotify_track in tidal_track.name.lower() or normalize(simple_spotify_track) in normalize(
        tidal_track.name.lower())
...
def match(tidal_track, spotify_track):
    return duration_match(tidal_track, spotify_track) and name_match(tidal_track, spotify_track) and artist_match(
...
def tidal_search(spotify_track_and_cache, tidal_session):
    ...
    if 'album' in spotify_track and 'artists' in spotify_track['album'] and len(spotify_track['album']['artists']):
        album_result = tidal_session.search('album', simple(spotify_track['album']['name']) + " " + simple(
            spotify_track['album']['artists'][0]['name']))
    ...
    for track in tidal_session.search('track', simple(spotify_track['name']) + ' ' + simple(
            spotify_track['artists'][0]['name'])).tracks:
        if match(track, spotify_track):
            return track
...
def get_tracks_from_spotify_playlist(spotify_session, spotify_playlist):
    ...
    results = spotify_session.playlist_tracks(spotify_playlist['id'],
                                              fields="next,items(track(name,album(name,artists),artists,track_number,duration_ms,id))")
    while True:
...
class TidalLikedSongsCache:
    ...
    def search(self, spotify_tracks):
        """ Add the cached tidal track where applicable to a list of spotify tracks """
        results = []
        cache_hits = 0
        for track in spotify_tracks:
...
def sync_playlist(spotify_session, tidal_session, spotify_id, tidal_id, config):
    ...
    tidal_track_ids = []
    spotify_tracks, cache_hits = TidalPlaylistCache(tidal_playlist, tidal_session).search(spotify_session,
                                                                                          spotify_playlist)
    if cache_hits == len(spotify_tracks):
        print("No new tracks to search in Spotify playlist '{}'".format(spotify_playlist['name']))
        return

    task_description = "Searching Tidal for {}/{} tracks in Spotify playlist '{}'".format(
        len(spotify_tracks) - cache_hits, len(spotify_tracks), spotify_playlist['name'])
    tidal_tracks = call_async_with_progress(tidal_search, spotify_tracks, task_description,
                                            config.get('subprocesses', 50), tidal_session=tidal_session)
    for index, tidal_track in enumerate(tidal_tracks):
        ...
        if tidal_track:
            ...
            print(color[0] + "Could not find track {}: {} - {}".format(spotify_track['id'], ",".join(
                [a['name'] for a in spotify_track['artists']]), spotify_track['name']) + color[1])
...
def get_playlists_from_spotify(spotify_session, config):
    ...
    while True:
        for spotify_playlist in spotify_results['items']:
            if spotify_playlist['owner']['id'] == config['spotify']['username'] and not spotify_playlist[
                                                                                            'id'] in exclude_list:
...
def sync_saved_songs(spotify_session, tidal_session, config):
    tracks = []
    results = spotify_session.current_user_saved_tracks()
    while True:
        tracks.extend([r['track'] for r in results['items'] if r['track'] is not None])
        # move to the next page of results if there are still tracks remaining in the playlist
        if results['next']:
            results = spotify_session.next(results)
        else:
    ...
    favorites = tidalapi.Favorites(tidal_session, tidal_session.user.id)
    cache = TidalLikedSongsCache(tidal_session)

    spotify_tracks, cache_hits = cache.search(tracks)

    if cache_hits == len(spotify_tracks):
        print("No new tracks in liked songs.")
        return

    task_description = f"Searching Tidal for {len(spotify_tracks) - cache_hits}/{len(spotify_tracks)} tracks"
    tidal_tracks = call_async_with_progress(tidal_search, spotify_tracks, task_description,
                                            config.get('subprocesses', 50), tidal_session=tidal_session)
    tidal_tracks, spotify_tracks = tidal_tracks[::-1], spotify_tracks[::-1]
    for index, tidal_track in enumerate(tidal_tracks):
        spotify_track = spotify_tracks[index][0]
        if tidal_track:
            favorites.add_track(tidal_track.id)
            print(f'Adding {tidal_track.name}')
        else:
            color = ('\033[91m', '\033[0m')
            print(color[0] + "Could not find track {}: {} - {}".format(spotify_track['id'], ",".join(
                [a['name'] for a in spotify_track['artists']]), spotify_track['name']) + color[1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--favorites', action='store_true', default=False, help='synchronize favorites')
    parser.add_argument('--playlists', action='store_true', default=False, help='synchronize playlists')
    ...
    if args.uri:
        ...
        sync_saved_songs(spotify_session, tidal_session, config)
    elif args.playlists:
